...
def discriminator_loss_fn(y_data, y_generated, data_label=0, label_noise=0.0):  # <--
    ...
    # loss = -( Dx - DGz ) = DGz - Dx
    loss_DGz = torch.mean(y_generated).to(device)
    loss_Dx = -torch.mean(y_data).to(device)
#     print(loss)
#     loss.requres_grad = True
    return loss_DGz + loss_Dx
    # ========================
...
def generator_loss_fn(y_generated, data_label=0):
    ...
    loss = -torch.mean(y_generated).to(device)
#     loss.requres_grad = True
    # ========================
    return loss
...
def train_batch(
    ...
    # ====== YOUR CODE: ======
    for _ in range(2):  # according to the paper, train the discriminator more than hte generator
        # WGAN - clip discriminator weights
        
        dsc_optimizer.zero_grad()
        x_gen = gen_model.sample(len(x_data), with_grad=True)    # with grad or not???????
        dsc_gen_pred = dsc_model(x_gen.detach()) # D(G(z))
        dsc_data_pred = dsc_model(x_data)  # D(x)
        dsc_loss = dsc_loss_fn(dsc_data_pred, dsc_gen_pred)
        dsc_loss.backward()
        dsc_optimizer.step()
        for param in dsc_model.parameters():
