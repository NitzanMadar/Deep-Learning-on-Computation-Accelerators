...
def random_labelled_image(
    ...
    # ====== YOUR CODE: ======
    image = torch.randint(low, high, shape, generator=None, out=None, dtype=dtype, layout=torch.strided, device=None, requires_grad=False)
...
def torch_temporary_seed(seed: int):
    ...
    # ====== YOUR CODE: ======
    tmp = torch.random.get_rng_state() # savve the current state into "tmp"
    # ========================
    try:
        # ====== YOUR CODE: ======
        torch.random.manual_seed(seed) # set the asked seed
        # ========================
        yield                          # finish the "__enter__" 
    finally:                           # start the "__exit__"
        # ====== YOUR CODE: ======
        torch.random.set_rng_state(tmp) # set back the "tmp" state
        # ========================
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        with torch_temporary_seed(index):
            ...
            image, image_class = random_labelled_image(self.image_dim, self.num_classes)
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        class ExampleIter():
            def __init__(self,obj):
                self.idx = 0
                self.img_dim =obj.image_dim
                self.num_classes = obj.num_classes

            def __next__(self):
                x = random_labelled_image(self.img_dim, self.num_classes)
                self.idx += 1
                return x
