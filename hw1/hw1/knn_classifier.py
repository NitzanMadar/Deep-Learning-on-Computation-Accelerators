...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            knn = torch.topk(dist_col, self.k, largest=False) # Returns the k smallest (largest=False) elements of the given input
            labels = self.y_train[knn.indices] #knn type is torch.return_types.topk(values=tensor(), indices=tensor())
            unique, counts = torch.unique(labels, return_counts=True)
            max_index = counts.max(0).indices
            # common pred:
            y_pred[i] = unique[max_index]
            # ========================
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    # ====== YOUR CODE: ======
    # 2AB^t
    ab2 = torch.mul(torch.mm(x1, x2.T), 2)
    # BB^t
    b2 = torch.mm(x2, x2.T)
    b_diag = torch.diag(b2, 0)
    bb = b_diag.expand(ab2.size(0), b2.size(0))
    # AA^t
    a2 = torch.mm(x1, x1.T)
    a_diag = torch.diag(a2, 0)
    aa = a_diag.expand(ab2.size(1), a2.size(0))
    aa = aa.T
    # sum
    dists = aa + bb - ab2
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = None
    # ====== YOUR CODE: ======
    accuracy = torch.eq(y, y_pred).sum().float() / y.size(0)
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        # ====== YOUR CODE: ======
        ratio = 1 / num_folds                         # if num_folds=4 -> 0.25
        size_valid = int(len(ds_train) * ratio)       # one element from k
        size_train = int(len(ds_train) - size_valid)  # k-1 other elements
        split_range = [size_train, size_valid]        # for torch.utils.data.random_split
        folds_list_accuracies = []                          # list of lists
        
        for fold in range(num_folds):                 # [0,1, ..., num_folds-1] , num_folds times...
            subset_train, subset_valid = torch.utils.data.random_split(ds_train, split_range) # split data randomaly
            # validation 
            dl_valid = torch.utils.data.DataLoader(subset_valid, shuffle=True)                # load validation
            validation_data, validation_labels = dataloader_utils.flatten(dl_valid)           # faltten - data and labels split
            # train 
            dl_train = torch.utils.data.DataLoader(subset_train, shuffle=True)                # load train
            model.train(dl_train) # train the model
            # predict the validation set
            prediction = model.predict(validation_data)
            # accuracy of the model's prediction
            fold_accuracy = accuracy(validation_labels, prediction)
            folds_list_accuracies.append(fold_accuracy)
            
        accuracies.append(folds_list_accuracies) # list of lists
        # ========================
