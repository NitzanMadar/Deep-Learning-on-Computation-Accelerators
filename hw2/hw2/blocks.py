...
class Sigmoid(Block):
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        dx = (self.grad_cache['sigmoid'] * (1.0 - self.grad_cache['sigmoid'])) * dout
        # ========================
...
class TanH(Block):
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        #\tanh(\vec{x}) = 1 - tanh(\vec{x})^2.
        dx =  (1-torch.square(self.grad_cache['tanh']))*dout
        # ========================
...
class CrossEntropyLoss(Block):
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        exp_x = torch.exp(x) # NxD
        sum_exp_x = torch.sum(exp_x, dim = 1).view(-1, 1) # Nx1
        grad = exp_x / (sum_exp_x*N)
        grad[range(N), y] += -1/N # d(xy)/d(xy)
        dx = grad * dout  # NxD
        # ========================
...
class Dropout(Block):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
#         print(torch.bernoulli(torch.full_like(x, self.p)))
        if self.training_mode:
            self.drop_mat = torch.bernoulli(torch.full_like(x, 1-self.p)) / (1-self.p)
            out = x * self.drop_mat # https://jmlr.org/papers/volume15/srivastava14a.old/srivastava14a.pdf
        else: # test
            out = x 
        # ========================
...
class Sequential(Block):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        din = dout #start with dout and apply reversed backward
...
class MLP(Block):
    ...
    def __init__(
        ...
        **kw,
        ...
        # ====== YOUR CODE: ======
        h_i_minus_1 = in_features # first layer is Dxh1
        for h_i in hidden_features:
            blocks.append(Linear(h_i_minus_1, h_i))
            h_i_minus_1 = h_i # update h_(i-1) for the next layer 
            # Activation:
            if activation == 'relu':
                blocks.append(ReLU())
            if activation == 'sigmoid':
                blocks.append(Sigmoid())
            # Dropout: #TODO: CHECK IF THE DROPOUT IS ADDEDE AFTER SIGMOID ALSO ?????????????????????????????????????
            if dropout>0:
                blocks.append(Dropout(dropout))
        # add last layer (hnxC):
        blocks.append(Linear(h_i_minus_1, num_classes))
