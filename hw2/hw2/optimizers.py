...
class VanillaSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            # dp already contain the CE gradient, we need to add the regularization gradient
            dp += self.reg*p        # add regularization gradient: d/dtheta{(0.5*lambda*norm(theta)^2)} = lambda*theta = self.reg*p
            p -= self.learn_rate*dp # thata = theta - learn_rate*gradient
            # ========================
...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.v = {}
        for i, (_, dp) in enumerate(params):
            self.v[i] = torch.zeros(dp.size())
        self.step_idx = 0
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg*p
            self.v[self.step_idx] = self.momentum*self.v[self.step_idx] - self.learn_rate*dp
            p += self.v[self.step_idx]
            self.step_idx += 1
        self.step_idx = 0
            # ========================
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg*p
            self.r[self.step_idx] = self.decay*self.r[self.step_idx] + (1.0 - self.decay)*torch.pow(dp,2)
            p -= self.learn_rate / (torch.sqrt(self.r[self.step_idx] + self.eps))  * dp
            self.step_idx += 1
        self.step_idx = 0
            # ========================
