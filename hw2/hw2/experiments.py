...
def run_experiment(
    ...
    **kw,
    ...
    classes_out = 10 # CIFAR, output
    first_img, _ = ds_train[0] # for input size only
    img_size = first_img.shape
    CE_loss = torch.nn.CrossEntropyLoss()
    channels = []
    for filters in filters_per_layer:
        channels = channels + [filters] * layers_per_block
#     print(kw)
    pooling_paprams = dict(kernel_size=2)
    conv_params = dict(kernel_size=3, stride=1, padding=1)
    pooling_params = dict(kernel_size=2)
    model = model_cls(img_size, classes_out, channels, pool_every, hidden_dims,
...
#     optimizer = torch.optim.RMSprop(model.parameters())
    trainer = training.TorchTrainer(model, CE_loss, optimizer, device)
