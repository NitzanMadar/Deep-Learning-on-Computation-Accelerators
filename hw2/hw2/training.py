...
#             train_loss.append(torch.mean(torch.tensor(train_res.losses)).item()) # good for plot without noise...
            train_loss.extend(train_res.losses)

            # test
            test_res = self.test_epoch(dl_test, **kw)
            test_acc.append(test_res.accuracy)
#             test_loss.append(torch.mean(torch.tensor(test_res.losses)).item()) # good for plot without noise...
            test_loss.extend(test_res.losses)
            
    
            #  epoch accuracy compare to previous epoch
            if actual_num_epochs>1:
                if test_acc[-1]<=best_acc: # < best accuracy
                    epochs_without_improvement += 1
                else:
                    epochs_without_improvement = 0
                    best_acc = test_acc[-1]
            
            else: #actual_num_epochs=1
                best_acc = test_acc[-1]
                
            # early stopping
            if epochs_without_improvement is not None and early_stopping is not None:
                if early_stopping <= epochs_without_improvement:
                    print('Early Stopping - Break')
                    break

#             print('epochs_without_improvement = ',epochs_without_improvement)
#             print('test_acc[-1] = ',test_acc[-1])
...
class BlocksTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        X= X.reshape(X.size(0),-1) # convert batch (Nx3x32x32) to Nx3*32*32
        # zero gradient first
        self.optimizer.zero_grad()
        #  - Forward pass
        model = self.model(X)
        loss = self.loss_fn(model, y)
        #  - Backward pass
        self.model.backward(self.loss_fn.backward())
        #  - Optimize params
        self.optimizer.step()
        y_pred = torch.argmax(model.data, 1)
        #  - Calculate number of correct predictions as num_correct (int, not tensor).
        num_correct = (torch.sum(torch.eq(y_pred, y).float())).item()
