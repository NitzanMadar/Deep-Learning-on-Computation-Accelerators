...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
        modules.append(nn.Conv2d(in_channels, 64, kernel_size = 5, stride = 2))
        modules.append(nn.BatchNorm2d(64))
        modules.append(nn.ReLU())
        
        modules.append(nn.Conv2d(64, 128, kernel_size = 5, stride = 2))
        modules.append(nn.BatchNorm2d(128))
        modules.append(nn.ReLU())
        
        modules.append(nn.Conv2d(128, 256, kernel_size = 5, stride = 2))
        modules.append(nn.BatchNorm2d(256))
        modules.append(nn.ReLU())
        
        modules.append(nn.Conv2d(256, out_channels, kernel_size = 1, stride = 1))
        modules.append(nn.BatchNorm2d(out_channels))
        modules.append(nn.ReLU())
        
#         modules.append(nn.Conv2d(512, out_channels, kernel_size = 5, padding = 2, stride = 2))
...
class DecoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
#         modules.append(nn.ConvTranspose2d(512, 256, kernel_size = 5, padding = 2, stride = 2, output_padding = 1))
#         modules.append(relu)
#         modules.append(nn.BatchNorm2d(512))

        modules.append(nn.ConvTranspose2d(in_channels, 256, kernel_size = 5, stride = 2))
        modules.append(nn.BatchNorm2d(256))
        modules.append(nn.ReLU())
        
        modules.append(nn.ConvTranspose2d(256, 128, kernel_size = 5, stride = 2))
        modules.append(nn.BatchNorm2d(128))
        modules.append(nn.ReLU())
        
        modules.append(nn.ConvTranspose2d(128, 32, kernel_size = 5, stride = 2))
        modules.append(nn.BatchNorm2d(32))
        modules.append(nn.ReLU())
        
        modules.append(nn.ConvTranspose2d(32, out_channels, kernel_size = 4))
        modules.append(nn.BatchNorm2d(out_channels))
#         modules.append(nn.ReLU())
...
#         self.n_features = n_features
        ...
        self.fc_reconstruct = nn.Linear(z_dim, n_features)
        # ========================
    ...
    def encode(self, x):
        ...
        # ====== YOUR CODE: ======
        features = self.features_encoder.forward(x).reshape((x.shape[0], -1))
        mu = self.fc_mu.forward(features)
        eps = torch.randn_like(mu)
        log_sigma2 = self.fc_log_sigma2.forward(features)
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        z = self.fc_reconstruct.forward(z)
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    data_loss = mse(x, xr)/(x_sigma2) 
    kldiv_loss = (z_log_sigma2.exp().sum(dim=1) + (z_mu ** 2).sum(dim=1) - z_mu.shape[1] - z_log_sigma2.sum(dim = 1)).mean()
    #
