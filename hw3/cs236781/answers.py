...
1. Since $\sigma$ is the variance it is a non negative quantity. by training $log(\sigma)$ we ensure that sigma is a non negative value, because taking the exponent of a number is a positive quantity.
The exponent of $log(\sigma)$ is $\sigma$.

2. It is easier to train and more stable - since using log, maps number from the interval [0,1] to [log(1), -inf], so instead of working with very small numbers in the interval [0,1], we would work with big numbers in the interval [log(1), -inf].
